...
class CM_HtmlConverter {
    ...
    public function convertHtmlToPlainText($html) {
        ...
        $html = preg_replace('/<div>/', '', $html);                         // IE Edge enter
        $html = preg_replace('/<\/div>/', PHP_EOL, $html);                  // same
        $html = preg_replace('/<p>/', '', $html);                           // IE 11 enter
        $html = preg_replace('/<\/p>/', PHP_EOL, $html);                    // same
